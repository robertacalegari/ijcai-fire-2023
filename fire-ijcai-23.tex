...
\author{
    Anonymous submission%ricordarsi di cambiare la ref per creepy e di aggiungere quelle per psyke
}
...
%
Comparisons performed by users and automated algorithmic comparisons can surely benefit from a unified scoring function encompassing both concepts of predictive performance and readability associated with the knowledge provided by SKE techniques.
%
...
Section \ref{sec:fire} contains the main contribution of this work and defines the \emph{fire score} for the evaluation of symbolic knowledge.
Section \ref{sec:use} and Section \ref{sec:experiments} discuss respectively the use of the metric as well as some first experiments.
...
%
To the best of our knowledge, not only an automatic way of evaluation is missing in the literature, but also the definition of a metric to synergically combine predictive performance and readability to measure the quality of the extracted knowledge as a unique indicator.
...
%
Accordingly, in \Cref{ssec:comp} several theoretical examples of the \fire{} score applications are explained, together with corresponding observations, whereas in \Cref{ssec:psi} more details about the $\psi$ parameter tuning are provided from a practical application standpoint.
